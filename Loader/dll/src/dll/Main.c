...
jclass findThreadClass(const char *name, jobject thread) {
    return (jclass) (*jniEnv)->CallObjectMethod(jniEnv,
                                                (*jniEnv)->CallObjectMethod(
                                                        jniEnv, thread, (*jniEnv)->GetMethodID(jniEnv,
                                                                                               (*jniEnv)->GetObjectClass(
                                                                                                       jniEnv, thread),
                                                                                               "getContextClassLoader",
                                                                                               "()Ljava/lang/ClassLoader;")),
                                                (*jniEnv)->GetMethodID(jniEnv, (*jniEnv)->FindClass(jniEnv,
...
classFileLoadHook(jvmtiEnv
    ...
    *
    ...
    if (class_being_redefined) {
        ...
        while (current != NULL) {
            if (!(*env)->
                    IsSameObject(env, current
                    ->clazz, class_being_redefined)) {
                previous = current;
...
JNICALL GetLoadedClasses(JNIEnv *env, jclass _) {
    jint classcount = 0;
    jclass *classes = NULL;
    (*jvmti)->GetLoadedClasses((jvmtiEnv *)
                                       jvmti, &classcount, &classes);
...
JNICALL GetClassBytes(JNIEnv *env, jclass _, jclass clazz) {
...
JNICALL RedefineClass(JNIEnv *env, jclass _, jclass clazz, jbyteArray classBytes) {
...
void loadJar(const char *path, jobject thread) {
    ...
    (*jniEnv)->CallVoidMethod(jniEnv, (*jniEnv)->CallObjectMethod(jniEnv, thread, (*jniEnv)->GetMethodID(jniEnv,
                                                                                                         (*jniEnv)->GetObjectClass(
                                                                                                                 jniEnv,
                                                                                                                 thread),
                                                                                                         "getContextClassLoader",
...
void Inject(JNIEnv *env, jvmtiEnv *jti, const char yolbi_dir[260]) {
    ...
    (*jvmti)->AddCapabilities((jvmtiEnv *)
                                      jvmti, &capabilities);
    ...
    (*jvmti)->SetEventCallbacks((jvmtiEnv *)
                                        jvmti, &callbacks, sizeof(jvmtiEventCallbacks));
    (*jvmti)->SetEventNotificationMode((jvmtiEnv *)
